...
# Practice set info ---- 
practice.begin("A2", learner="Kevin Baron", email="baronk2@uw.edu")
...
# install.packages("stringr") # Already installed on my device.

library("stringr")

# 1b: Load the data from https://countlove.org/data/data.csv (Variable: `protests`)

count_love_df <- read.csv("https://countlove.org/data/data.csv",
                          header = TRUE,
...
num_protests <- nrow(count_love_df)
print(num_protests)
...
num_features <- ncol(count_love_df) # or length(count_love_df)
print(num_features)
...
num_attendees <- count_love_df$Attendees
print(num_attendees)
...
locations <- count_love_df$Location
...
count_in_location <- function(location) {
  ...
  n <- length(locations[str_detect(locations, location)])
...
missing_summary <- count_in_location("Manson, WA")
...
# `state_summary` has one more protest counted for "DC" than `dc_summary`. There
# might have been a protest in "DC" that was not listed in "Washington, DC". As
# far as I know, there aren't any other cities or municipalities, but maybe one
# location is listed simply as "DC" since most people know what that means.
...
dates <- as.Date(count_love_df$Date)
...
in_2020 <- dates[dates >= "2020-01-01" & dates < "2021-01-01"]

# 4f: Create a vector of the dates that are in 2019. (Variable: `in_2019`)

in_2019 <- dates[dates >= "2019-01-01" & dates < "2020-01-01"]
...
#    May 31st, 2020? (Variable: `num_on_may_31`)
...
# `months()` is not working for me. Instead, I will use str_sub() to get the
# month number

# months <- str_sub(in_2020, 6, 7)

months <- months(in_2020)

by_month_table <- table(months)
...
months_2020 <- months
num_july_2020 <- length(months_2020[months_2020 == "July"])

months_2019 <- months(in_2019)
num_july_2019 <- length(months_2019[months_2019 == "July"])


change_july_protests <- abs(num_july_2020 - num_july_2019)
...
purpose <- count_love_df$Event..legacy..see.tags.
...
# `gsub()` solution from MrFlick on StackOverflow:
# https://stackoverflow.com/questions/24173194/remove-parentheses-and-text-within-from-strings-in-r

# There is an inconsistency in the original data in record 13758 where
# "Collective Bargaining ()" is listed as a value for `Event..legacy..see.tags.`.
# The outer nesting of the `str_replace()` function handles this single case.
# An `|` operator could have also worked instead of nesting.

get_purposes <- function() {
  
  # Solution using `str_replace()` and `[:print:]` regular expression not working
  
  # return(substr(purpose, 1, str_detect(purpose, "(")))
  
  # return(str_replace(purpose, " ([:alpha:])| ()", ""))
  ...
  # return(str_replace(gsub("\\s*\\([^\\)]+\\)", " ()", purpose), " ()", ""))
}
...
#    `purpose`          A string for the protest purpose name (e.g., "Civil Rights")
...
write_report <- function(md_doc, fname="~/Kevin/School/University_Of_Washington/Informatics/2022-2023/Autumn_2022/Info_201/info201/reports/report.md") {
...
# Already done on my device
# install.packages("dplyr")
library("dplyr")

# Filters by using `grepl()` to match text of `purpose` and `position_taken`
...
filter_positions <- function(purpose, position_taken = NULL) {
  ...
  if (is.null(position_taken)) {
    return(filter(df, grepl(purpose, df$Tags, fixed = TRUE)))
  } else {
  
  return(filter(df, grepl(purpose, df$Tags, fixed = TRUE),
                grepl(position_taken, df$Tags, fixed = TRUE)))
  }
}
...
# Input:
#
#    `purpose`          A string for the protest category name (e.g., "Civil Rights")
#    `position_taken`   A string of the position taken (e.g., "For women's rights").
#                       If NULL, then the data has NOT been filtered by a position_taken 
#
# The function passes `purpose` and `position_taken` directly into
# `filter_positions()`, which returns the dataframe as a the first parameter
# into `format_doc()`, and also passes `purpose` and `position_taken` as 
# second and third parameters into `format_doc()`. The Markdown file returned
# by `format_doc()` is returned up through this function.
#
#
#
# Initially, I was using the `write_report()` function within this function to 
# also write the report, but then I realized that for grading purposes, you may
# be using your own `write_report()` function to write a file onto a different
# hard-coded file location to see our reports. In that version, the Markdown
# document was passed as the first parameter to `write_report()`. The
# second parameter of `write_report()` is already hard-coded within the function
# declaration as the appropriate file location and file name to my computer.
# The default file in the original master copy of this `.R` file suggested hard
# coding the file name, but I could see why we might want to code a relative path for this
# assignment so that files could be created and viewed in an appropriate folder
# on a TA's device.
#
# To avoid any of this confusion, I have kept things simply and simply returned
# the Markdown document in this function.
#
# Output:
# 
#    A formatted Markdown document.
#
# The output report takes this form:
#
#     # Protest purpose:  Environment 
#     ## Position taken:  For renewable energy 
#     _Summary_
...
filter_and_report <- function(purpose, position_taken = NULL) {
  ...
  return(format_doc(filter_positions(purpose, position_taken), purpose, position_taken))
...
write_report(filter_and_report("Civil Rights"))
